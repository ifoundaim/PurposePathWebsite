...
- Make gold button and menu shimmer animations loop seamlessly by aligning gradient tiling with matching first/last keyframe positions in `app/globals.css` and `tests/site.spec.ts`.
- Continue the homepage founder-support gradient through the footer subscribe area and cover the transition with a Playwright regression in `tests/site.spec.ts`.
